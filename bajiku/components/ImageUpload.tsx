...
// const ImageUpload = ({ onImageSelected }: { onImageSelected: (imageUri: string) => void }) => {
...
interface ImageUploadProps {
    ...
    currentImage?: string | null; // Prop to pass the current image URL
}

const ImageUpload: React.FC<ImageUploadProps> = ({ onImageSelected, currentImage }) => {
    const [imageUri, setImageUri] = useState<string | null>(currentImage || null);// Initialize with the current image
    const [hasMediaLibraryPermission, setHasMediaLibraryPermission] = useState<boolean | null>(null); 
    const [hasCameraPermission, setHasCameraPermission] = useState<boolean | null>(null);
    const [modalVisible, setModalVisible] = useState(false);
    ...
    const pickImage = async () => {
        ...
        if (!result.canceled && result.assets && result.assets.length > 0) {
            const uri = result.assets[0].uri;
            setImageUri(uri);
            onImageSelected(uri);  // Pass the new image URI back to parent
            setModalVisible(false); 
        }
    };
